...
def elp_mpp02_initialise(mode):
    ...
    if(mode==1):  # DE 405 / DE 406
       w(1,3) = w(1,3) - 0.00018865/r2as
       w(1,4) = w(1,4) - 0.00001024/r2as
       
       w(2,2) = w(2,2) + 0.00470602/r2as
       w(2,3) = w(2,3) - 0.00025213/r2as
       
       w(3,2) = w(3,2) - 0.00261070/r2as
       w(3,3) = w(3,3) - 0.00010712/r2as
       ...
       for iLine in range(nmpb[iFile,1]):  # do iLine=1,nmpb(iFile,1)
          ...
          for k in range(5):  # do k=0,4
             ...
             for i in range(4)+1:  # do i=1,4
                fmpb(k,ir) = fmpb(k,ir) + ilu(i)*dela(i,k)
          
          if(iFile==3): fmpb(0,ir)=fmpb(0,ir)+pio2
        ...
        for it in range(4):  # do it=0,3
            ...
            for iLine in range(nper(iFile,it,1)):  # do iLine=1,nper(iFile,it,1)
                ...
                for k in range(5):  # do k=0,4
                    ...
                    for i in range(4)+1:  # do i=1,4
                        fper(k,ir) = fper(k,ir) + ifi(i)*dela(i,k)
